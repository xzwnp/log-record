...
//				}
                log.info("操作名称: {} ,traceId: {} ,操作描述: {} ,耗时: {}ms,请求参数:{},返回值: {}", operateName, dto.getTraceId(), content, cost, JsonUtils.toJson(dto.getParameterMap()), returnValueString);
            } else {
                log.info("操作名称: {} ,traceId: {} ,操作描述: {} ,耗时: {}ms,请求参数:{}", operateName, dto.getTraceId(), content, cost, JsonUtils.toJson(dto.getParameterMap()));
            }
            //日志持久化
            saveOperateLog(dto);
        } catch (LogRecordException e) {
