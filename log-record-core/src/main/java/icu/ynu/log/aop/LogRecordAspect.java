...
//	}
	...
	public Object around(ProceedingJoinPoint joinPoint, LogRecord logRecord) throws Throwable {
		...
		try {
			...
			if (logRecord.executeBefore()) {
				...
				doParseExpression(logRecordDto, logRecord, signature.getMethod(), joinPoint.getArgs());
			}
	...
	private void doParseExpression(LogRecordDto dto, LogRecord logRecord, Method method, Object[] args) {
		EvaluationContext evaluationContext = prepareLogRecordEvaluationContext(method, args);
		dto.setBizId(parseExpression(logRecord.bizId(), evaluationContext));
		dto.setContent(parseExpression(logRecord.content(), evaluationContext));
	}
